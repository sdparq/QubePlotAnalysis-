...
export const DEFAULT_WALK_HYPERREAL_PROMPT = `TASK: Turn this rough grey 3D viewport capture into a HYPERREALISTIC eye-level PHOTOGRAPH of a real, lived-in city street in Dubai / Abu Dhabi — indistinguishable from a picture taken on a full-frame camera. The input is only a massing and proportion guide (a clay model). The finished image must show NO trace of the 3D-model look: no flat untextured surfaces, no plain boxes, no empty lifeless streets, no CGI plastic feel.
...
PROJECT BUILDING (the tower the camera faces, the one with the detailed facade): keep its silhouette, footprint, storey count, podium step and every set-back exactly where the input puts them — then DESIGN it like a world-class architect instead of leaving it a maquette. A premium contemporary residential tower with real depth and layering in the facade: full-height glazing in dark anodised frames with believable interior life glimpsed through the glass (sheer curtains, pendant lights, plants, furniture silhouettes), deeply carved balcony loggias with clear-glass balustrades, warm timber soffits and residents' planting spilling over some balconies, crisp floor-slab shadow lines, off-white architectural concrete or stone-clad piers with visible material grain. Ground / podium levels become a generous double-height arrival: bronze-anodised vertical fins or brise-soleil, a stone-framed lobby portal glowing warmly from inside, discreet canopy, landscaped edge. Real reflections of sky, street and neighbouring towers move across the glazing.
