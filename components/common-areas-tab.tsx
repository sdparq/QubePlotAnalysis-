            ...
            <tbody>
              {project.commonAreas.map((c) => (
                <tr key={c.id}>
                  ...
                  <td>
                    <input type="number" min={1} step={1} className="cell-input text-right w-20" value={c.floors} onChange={(e) => update(c, { floors: Math.max(1, Math.round(parseFloat(e.target.value) || 1)) })} />
                  </td>
                  <td className="text-right">{fmt2(c.area * c.floors)}</td>
                  <td>
                    <input type="checkbox" checked={c.countAsGFA} onChange={(e) => update(c, { countAsGFA: e.target.checked })} />
                  </td>
                  <td>
                    <input className="cell-input" value={c.notes ?? ""} onChange={(e) => update(c, { notes: e.target.value })} />
                  </td>
                  <td className="text-right">
                    <button className="btn btn-danger text-xs" onClick={() => { if (confirm(`Delete ${c.name}?`)) remove(c.id); }}>Delete</button>
                  </td>
                </tr>
              ))}
              <tr className="font-semibold bg-slate-50">
                <td colSpan={3}>Subtotal — counts as GFA</td>
                <td className="text-right">{fmt2(totalGFA)}</td>
                <td colSpan={3}></td>
              </tr>
              <tr className="bg-slate-50">
                <td colSpan={3}>Subtotal — non-GFA (open air)</td>
